...
2. **`window-batch/start_health_check_background.bat` 편집**
...
```bash
# 성공 시에도 서버 응답 내용을 확인하고 싶을 때
python health-check.py --url http://your-api.com/health --verbose
```

```bash
# 모든 옵션 조합
python health-check.py -u http://your-api.com/health -v -a "서버 다운됨"
```
...
| `--verbose`  | `-v`      | 상세 로그 모드 (성공 시에도 응답 내용 표시) | 비활성화                           |
...
- **백그라운드 실행**: `window-batch/stop_health_check.bat` 실행

### 방법 5: Windows 시작 프로그램으로 등록 (자동 실행)

컴퓨터가 부팅될 때 자동으로 실행되도록 설정:

1. `window-batch/start_health_check_background.bat` 파일 우클릭
2. "바로 가기 만들기" 선택
