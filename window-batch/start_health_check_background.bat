...
echo 종료하려면 작업 관리자에서 python.exe 프로세스를 종료하거나
echo stop_health_check.bat을 실행하세요.
echo.
echo 3초 후 이 창은 자동으로 닫힙니다...
timeout /t 3 > nul

REM 백그라운드에서 Python 실행
start /B pythonw health-check.py --url %API_URL% --interval %CHECK_INTERVAL% --timeout %TIMEOUT% --alert "%ALERT_MESSAGE%" %VERBOSE%
